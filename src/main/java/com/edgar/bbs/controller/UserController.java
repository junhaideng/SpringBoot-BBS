...
public class UserController {
    ...
    public List<MessageInfo> getMessage(HttpSession session) {
        ...
        MessageSettingsInfo messageSettingsInfo = messageSettingsDao.findMessageSettingsUsingUsername(username);
        String comment = messageSettingsInfo.isComment() ? "评论" : "comment"; // comment 实际上类似占空，无实意
        String like = messageSettingsInfo.isLike() ? "点赞" : "like";
        String star = messageSettingsInfo.isStar() ? "收藏" : "star";
        return messageDao.getAllByUsername(username, comment, like, star);
    }
    ...
    public Map getUnreadMessage(HttpSession session) {
        ...
        String like = messageSettingsInfo.isLike() ? "点赞" : "like";
        String star = messageSettingsInfo.isStar() ? "收藏" : "star";
        Long num = messageDao.getUnreadMessage(username, comment, like, star);
